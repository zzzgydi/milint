...
def checkNetPath(path):
    ...
    if not requests.get(path):
        print(requests.get(path))
        return False
    return True
...
def printInfoMsg(level, msg, ffile, tfile):
    print("[%s]: %s" % (level.capitalize(), msg))
    print("  From:", ffile)
    print("    To:", tfile)
    ...
    try:
        ...
        for each in workstack:
            ...
            if not os.path.isfile(each['src']):
                printErrorMsg('Error', 'Source Image not found.', filename,
                              each['line'])
                continue
            printInfoMsg('Info', 'Copying Image...', each['src'], each['dst'])
        ...
        print('[Error]: UnicodeEncodeError - Please use utf-8...')
...
if __name__ == "__main__":
    help()
    print(os.path.abspath(c_workdir))
    mainRun('D:\Test')
